...
(def schema-tx (slurp (io/resource "cashmgmt/accounts-schema.dtm")))
...
(def data-tx  (slurp (io/resource "cashmgmt/one-account.dtm")))
...
(d/transact conn [{:db/id (d/tempid :db.part/user)
                   :account/name "issuer"
                   :account/balance 0M
                   :account/max-balance 0M
                   :account/min-balance -100M}])
...
(mapv first alice-txs)
(query/e (ffirst alice-txs))
(query/e alice)
(query/qfs '[:find ?txs :where [?e :account/transactions ?txs]] (db conn))
(query/qes (map ffirst alice-txs))
(first alice-txs)
...
;; C-u C-space - go to previous location