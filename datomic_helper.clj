...
;(comment ;; Stick everything in comment so it doesn't run in swank
  ...
  ;; Define URI
  ;; ---------
  ;; Persistent Dev setup, requires setting up transactor with
  ;; config/samples/dev-transactor.
  ;; properties
  ...
  (def datomic-path "/Users/dev/datomic/datomic/")
  (do
    ;; Setup sample schema
    (.get (.transact conn
                     (first (Util/readAll
